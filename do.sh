...
shell() {
    _requires web_run
    exec -w /code/bookshelf web_run /bin/bash
}

migrate() {
    _requires web_run
    exec -w /code/bookshelf web_run ./manage.py migrate "$@"
}

makemigrations() {
    _requires web_run
    exec -w /code/bookshelf web_run python manage.py makemigrations "$@"
}

check() {
    _requires web_run
    exec -w /code/bookshelf web_run ./manage.py check
}
